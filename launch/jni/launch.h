...
int NDKINJECT::find_pid_of(char *process_name)
{
	int id;
	pid_t pid = 0;
	DIR* dir;
...
int NDKINJECT::inject_remote_process(pid_t target_pid, const char *library_path, const char *function_name, const char *param, size_t param_size, int bUnload)
{
	...
	mmap_addr = get_remote_addr(target_pid, libc_path, dlsym(dlopen("/system/lib/libc.so", RTLD_LAZY), "mmap"));
